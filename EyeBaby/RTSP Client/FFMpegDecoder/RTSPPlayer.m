...
-(id)initWithVideo:(NSString *)moviePath usesTcp:(BOOL)usesTcp parent:(id)prnt
{
    ...
    m_videoPacketQueue = [[NSMutableArray alloc] init];
    videoPacketQueueLock = [[NSLock alloc] init];
...
- (BOOL)stepFrame
{
    ...
    {
        readPacket = true;
        packet.pts = av_rescale_q( packet.pts, pFormatCtx->streams[packet.stream_index]->time_base, _audioStream->time_base );
        if(packet.stream_index==videoStream)
        {
            if (m_videoPacketQueue.count < 150)
            {
                [videoPacketQueueLock lock];
                [m_videoPacketQueue addObject:[NSMutableData dataWithBytes:&packet length:sizeof(packet)]];
                [videoPacketQueueLock unlock];
            }
        }
        else if (packet.stream_index==audioStream)
        {
            if (_audioController->m_curBufIdx < AudioQueueCount)
            {
                if (_audioController->m_curBufIdx == 0)
                    [_audioController ResetQueue];
                [_audioController ReadyBuffer:&packet];
            }
        }
	}
    return readPacket;
...
{
    [videoPacketQueueLock lock];
    if (m_videoPacketQueue.count > 0)
    {
        AVPacket* videoPkt;
        NSMutableData *packetData = [m_videoPacketQueue objectAtIndex:0];
        videoPkt = [packetData mutableBytes];
        if ((int)(videoPkt->pts - pts) < 20000)
        {
            int frameFinished = 0;
            avcodec_decode_video2(pCodecCtx, pFrame, &frameFinished, videoPkt);
            [m_videoPacketQueue removeObjectAtIndex:0];
            if (frameFinished != 0)
                [m_parent performSelectorOnMainThread:@selector(displayNextFrame) withObject:Nil waitUntilDone:NO];
        }
    }
    [videoPacketQueueLock unlock];
}
